...
CODE_EXTENSIONS = {
    ...
    '.hpp', '.cs', '.go', '.rb', '.php', '.sh', '.cc', '.cxx', '.hxx',
    '.csproj', '.sln'
}
...
FUNCTION_PATTERNS = {
    ...
    'shell_function': r'(?:^|\s+)(\w+)\s*\(\s*\)\s*{',
    'shell_command': r'(?:^|\s+)function\s+(\w+)\s*(?:\(\s*\))?\s*{',
    'cpp_method': r'(?:^|\s+)(?:virtual\s+)?(?:[\w:]+\s+)?(\w+)\s*\([^)]*\)\s*(?:const\s*)?(?:override\s*)?(?:final\s*)?(?:noexcept\s*)?(?:=\s*0\s*)?{?',
    'csharp_method': r'(?:^|\s+)(?:public|private|protected|internal|async\s+)?\s*(?:virtual\s+)?(?:override\s+)?(?:[\w<>]+\s+)?(\w+)\s*\([^)]*\)\s*(?:where\s+[^{]+)?{?'
}
